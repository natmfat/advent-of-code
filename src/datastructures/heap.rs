  ...
  pub fn pop(&mut self) -> Option<i32> {
    ...
    while idx < len {
      ...
      if left_child_idx < len
        && self.values.get(left_child_idx).unwrap() > self.values.get(largest_child_idx).unwrap()
      {
        largest_child_idx = left_child_idx;
      }

      if right_child_idx < len
        && self.values.get(right_child_idx).unwrap() > self.values.get(largest_child_idx).unwrap()
      {
